...
export const Button = (p: PropsWithChildren<Props>) => {
	...
	const onMouseEnterHandler = () => {
		if (canvasRef.current) {
			...
			const animate = () => {
				if (y < bounds.height) {
					context.fillStyle = `rgb(${randomNumber(150, 255)}, ${randomNumber(150, 255)}, ${randomNumber(
						150,
						255
					)})`;
					if (x < bounds.width) {
	...
	return (
		<ButtonContainer
            ...
            onClick={() => {
                window.open('https://hh.ru/resume/e50891a3ff0ebe71940039ed1f4b424d456c38', '_blank')
            }}
			theme={theme}>
			<AbsoluteBlock>
				<AnimatedCanvas width={bounds.width} height={bounds.height} ref={canvasRef} />
			</AbsoluteBlock>
